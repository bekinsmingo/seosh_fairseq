...
class FairseqTask(object):
    ...
    def train_step(
        ...
        #     Tra()
        #     '''
        #     (Pdb) sample['net_input']['src_tokens'].size(); sample['net_input']['src_tokens'][0];
        #     torch.Size([56, 173])

        #             429,  159,   93,  107, 3684,   20,  114,   46, 2519,  237,   35,  173,
        #             6,    4,  430, 1164,    1], device='cuda:0')
        #     (Pdb) sample['net_input']['src_tokens'].size(); sample['net_input']['src_tokens'][-1];
        #     torch.Size([56, 173])

        #             99, 4380,   47,   54, 7247,   29,    8, 3587, 4815,   73,    6,   38,
        #             89,  167,  121,   28, 1523], device='cuda:0')

        #     (Pdb) sample['target']
        #     tensor([[  12,   14, 1656,  ..., 1164,    2,    1],
        #             [  99, 3027, 3417,  ...,   85,    2,    1],
        #             [4050,   19,    4,  ...,   69,    2,    1],
        #             ...,
        #             [   4, 6133,   53,  ..., 1626,  628,    2],
        #             [ 181,  800,   11,  ...,   16,  338,    2],
        #             [  51,   47, 1212,  ...,   28, 1523,    2]], device='cuda:0')
        #     '''
        ...
        dict_keys(['src_tokens', 'src_lengths'])

        (Pdb) sample['net_input']['src_tokens'].size(); sample['net_input']['src_tokens'][0];
        torch.Size([344, 29])
        tensor([   2,   40,   13,    4, 2336,   62,   28, 4444,   61,    6,    8,  273,
                7,  276,   11,   41,   35, 1303,   29,    7,  107,    8,  537,   85,
                34,   18,  256,  191, 3163], device='cuda:0')
        ...
        # for roberta
        '''
        (Pdb) len(self.target_dictionary)
        221457
        (Pdb) self.target_dictionary.string(sample['net_input']['src_tokens'][0])
        "<mask> had wept herself to sleep and she had reason too <mask> <mask> young companions had laughed at her and told her that she was to have for a husband an <mask> <mask> without a nose <mask> probably <mask> many as fifty were footer <mask> <mask> into the margny of the wax model some into the shoulders <mask> into <mask> trunk some upwards through the soles of the feet till <mask> figure was completely permeated with pins but <mask> can be no two opinions <mask> your <mask> of such a song that's the sort of arrow to go straight <mask> the heart of the public a world wide public and if <mask> <mask> the first to say <mask> to you i hope you <mask> one day remember <mask> <mask> my <mask> as she was laid down her left hand fell helpless over the side of the bed <mask> there was a dominating quality about the man not to be gainsaid but to let judith face it all <mask> challenged jane these <mask> study periods were boring for fuzzy who wasn't <mask> interested in the oxygen exchange mechanism of the intelligent beetles <mask> aldebaran vi now imagine that the <mask> has become a <mask> and has <mask> son who is exposed to the same temptations and has companions who lead him ashery every sort of iniquity and parents and friends who try to keep him right <mask> you've got to find yourself and i've got <mask> find myself and when we've both done that we shall know how we stand to one another for repairmen i was convinced that if i could only count consecutive numbers long enough without losing one i should suddenly on <mask> some far distant <mask> find myself in possession of the great secret yes always <mask> that <mask> <mask> <mask> <mask> easier <mask> live without you rev'rence answered rather <mask> garder for scents like musical sounds are rare <unk> of <mask> essence of memory this is a prodigious fine phrase i hope <mask> means <mask> and scents need not be seductive in themselves to recall priestley's seductions of <mask> and days gone by the yarn <mask> am now going to spin you is how <mask> george hill won <mask> while employed as a scout on the east coast <mask> meetuck he's <mask> quick for you only think well before you wreck my life hermes to latona because speech investigates and <mask> <mask> oblivion tenny's contrary to these i <mask> to have sent my second and third letters by n and in fact had actually packed them with his things <mask> a fine picture miss <mask> <mask> chance <mask> did <mask> in the slightest accord with the whole life of the baron before and since and <mask> <mask> <mask> people hesitate to speak and which one should not <mask> from the past in which it <mask> buried"
        '''
