...
class MemoryTransformerXL(nn.Module):
    def __init__(self, target_dictionary, dim, seq_len, depth, emb_dim = None, memory_layers = None, mem_len = None, lmem_len = None, \
                heads = 8, gru_gated_residual = True, mogrify_gru = False, attn_dropout = 0., attn_layer_dropout = 0., \
                ff_glu = False, ff_dropout = 0., \
                one_kv_head = False, num_mem_kv = 0, mem_write_iters = 2, \
                adaptive_input = False, adaptive_input_factor=4, adaptive_input_cutoff=None, \
                adaptive_softmax = False, adaptive_softmax_factor=4, adaptive_softmax_cutoff=None, adaptive_softmax_dropout=0, \
        ...
        num_tokens = len(target_dictionary)
        ...
        self.token_emb = nn.Embedding(num_tokens, emb_dim, target_dictionary.pad())
        ...
        if adaptive_input:
            embed_tokens = AdaptiveInput(
                ...
                initial_dim = emb_dim,
                factor = adaptive_input_factor,
                output_dim = emb_dim,
                cutoff = options.eval_str_list(adaptive_input_cutoff, type=int),
                q_noise = quant_noise_pq,
                qn_block_size = quant_noise_pq_block_size,
            )
            self.token_emb = embed_tokens
        ...
        if adaptive_softmax:
            self.adaptive_softmax = AdaptiveSoftmax(
                ...
                adaptive_inputs=embed_tokens if tie_adaptive_weights else None,
                factor=adaptive_softmax_factor,
                tie_proj=tie_adaptive_proj,
            )
            # self.to_logits = self.adaptive_softmax
